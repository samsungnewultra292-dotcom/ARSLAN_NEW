...
import {
  ...
  type BroadcastSendingMode,
} from '@/lib/whatsapp/sending-mode';
...
export interface CreateBroadcastParams {
  ...
  /**
   * Sending mode for the fan-out. Super Safe paces every outbound
   * message at ~8/min through the shared token bucket; Instant (the
   * default) sends as fast as Meta's own limits allow. Not persisted
   * here (the atomic create RPC takes no such argument) — the delivery
   * loop reads it straight off the plan.
   */
...
export interface BroadcastPlan {
  broadcastId: string;
  /** Account that owns the broadcast — the scope the Super Safe token
   *  bucket is keyed on, so all of one account's sends share the rate. */
  accountId: string;
  templateName: string;
  ...
  rejected: number;
  /** Sending mode for {@link deliverBroadcast}. */
  mode: BroadcastSendingMode;
}
...
export async function createBroadcast(
  ...
  return {
    ...
    planned,
    rejected,
    mode: isBroadcastSendingMode(params.mode)
      ? params.mode
      : DEFAULT_BROADCAST_SENDING_MODE,
  };
}
...
export async function deliverBroadcast(
  ...
  // Super Safe mode paces each Meta request through the shared token
  // bucket (~8/min per account). This runs server-side for the public
  // API and resume fan-outs, so the cap survives the browser tab the
  // wizard started the campaign in being closed.
  const superSafe = plan.mode === 'super_safe';
