...
function InboxPageInner() {
  ...
  // Synchronous mirrors of the conversations list + active contact so the
  // contact_tags realtime handler below can decide what to refresh without
  // a stale closure reading an old render's state.
    ...
    try {
      ...
      setConversations((prev) => {
        ...
        if (existing) {
          ...
          // the row we just read). Only refresh `contact`, which the
          // realtime payloads never carry — this is also what pulls
          // newly assigned/removed `contact.tags` into the list badges.
          return prev.map((c) =>
            c.id === fetched.id
  ...
  /**
   * Keep the conversation list's tag badges fresh when a tag is assigned /
   * removed anywhere else — another agent's tab, the Contacts page, an
   * automation or a flow writing to contact_tags. Realtime payloads carry
   * only the join row, so we re-hydrate the affected conversation(s) and,
   * if the visible contact is involved, refresh its tags too.
   */
  const handleContactTagEvent = useCallback(
    (event: {
      eventType: string;
      new: Partial<ContactTag>;
      old: Partial<ContactTag>;
    }) => {
      const contactId =
        event.eventType === "DELETE" ? event.old?.contact_id : event.new?.contact_id;
      if (!contactId) return;

      for (const c of conversationsRef.current) {
        if (c.contact_id === contactId) hydrateConversation(c.id);
      }
      ...
      if (activeContactRef.current?.id === contactId) {
        ...
        supabase
          ...
          .then(({ data }) => {
            if (!data) return;
            const { contact_tags, ...rest } = data as Contact & {
              contact_tags?: { tags: Tag | null }[];
            };
            const tags = (contact_tags ?? [])
              .map((ct) => ct.tags)
              .filter((tg): tg is Tag => tg != null);
            setActiveContact({ ...rest, tags });
            setConversations((prev) =>
              prev.map((c) =>
  ...
  /**
   * Same-tab tag edits made from the right-side Contact sidebar. The
   * sidebar already persisted the change via the existing tag API and
   * re-read the rows; this just mirrors the fresh list back onto both the
   * conversation-list badges and the active contact. No second mechanism —
   * the sidebar's `onTagsChange` payload IS the account's tag data.
   */
  const handleSidebarTagsChange = useCallback((contactId: string, tags: Tag[]) => {
    setConversations((prev) =>
      prev.map((c) =>
        c.contact?.id === contactId && c.contact
          ? { ...c, contact: { ...c.contact, tags } }
          : c,
      ),
    );
  ...
  const { isConnected } = useRealtime({
    ...
    onContactTagEvent: handleContactTagEvent,
    enabled: true,
  });
